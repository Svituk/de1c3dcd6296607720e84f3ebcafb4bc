...
$title = 'APICMS - '.display_html($theme_name['name']).'';
...
if (isset($user['id']) && !empty($_POST['txt']) && csrf_check()){
	...
	if (empty($post_error)){
		...
		$theme_id_val = intval($theme_id);
		mysqli_stmt_bind_param($stmt, 'siii', $raw_text, $user_id_val, $theme_id_val, $time_now);
...
if ($theme_name['close']==0 && ($is_user && ($theme_name['id_user']==$user_id || $user_level==1 || $user_level==2)))echo '<td width="25%"><center><a href="theme_close.php?id='.$theme_name['id'].'"><img src="/design/styles/'.display_html($api_design).'/forum/theme_close.png"></a></center></td>';
if ($theme_name['close']==1 && ($is_user && ($theme_name['id_user']==$user_id || $user_level==1 || $user_level==2)))echo '<td width="25%"><center><a href="theme_open.php?id='.$theme_name['id'].'"><img src="/design/styles/'.display_html($api_design).'/forum/theme_open.png"></a></center></td>';
echo '<td width="25%"><center><a href="theme.php?id='.$theme_name['id'].'&'.$reloads.'"><img src="/design/styles/'.display_html($api_design).'/forum/reload.png"></a></center></td> ';
if ($theme_name['close']==0 && ($is_user && ($theme_name['id_user']==$user_id || $user_level==1 || $user_level==2)))echo '<td width="25%"><center><a href="del_theme.php?id='.$theme_name['id'].'"><img src="/design/styles/'.display_html($api_design).'/forum/del_theme.png"></a></center></td>';
if ($theme_name['close']==0 && ($is_user && ($theme_name['id_user']==$user_id || $user_level==1 || $user_level==2)))echo '<td width="25%"><center><a href="edit_theme.php?id='.$theme_name['id'].'"><img src="/design/styles/'.display_html($api_design).'/forum/edit_theme.png"></a></center></td>';
...
echo "<br/> <b>".apicms_smiles(apicms_br(display_html($post_theme['text'])))."</b></td></tr></table></div>";
}
...
echo "<br/> <b>".apicms_smiles(apicms_br(display_html($post_post['text'])))."</b>";
...
if ($in_theme > 0)echo '<div class="apicms_subhead"><center> <img src="/design/styles/'.display_html($api_design).'/forum/in_theme.png"> Сейчас в теме "'.$theme_name['name'].'": '.$in_theme.' чел.</center></div>';
