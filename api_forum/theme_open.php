...
// If user is not logged in, redirect immediately before any output
...
if ($theme_id && $check_theme_row['cnt']==1){
    ...
    if ($post && $post['close']==1 && (isset($user['id']) && ($user['id'] == $post['id_user'] || $user['level']==1 || $user['level']==2))){
        if ($_SERVER['REQUEST_METHOD']==='POST'){
            if (csrf_check()){
                mysqli_query($connect, "UPDATE `api_forum_theme` SET `close` = '0' WHERE `id` = '".intval($post['id'])."' LIMIT 1");
            ...
            require_once '../design/styles/'.htmlspecialchars($api_design).'/head.php';
            echo "<div class='apicms_subhead'><center>Подтвердите открытие темы</center></div>";
            echo "<form method='post' action='?id=".$theme_id."'>";
            echo "<input type='hidden' name='csrf_token' value='".htmlspecialchars(csrf_token())."' />";
            echo "<div class='apicms_subhead'><center><input type='submit' value='Открыть' /></center></div>";
            echo "</form>";
            require_once '../design/styles/'.htmlspecialchars($api_design).'/footer.php';
            exit();
        }
...
// Now include head and render page (no more redirects expected)
require_once '../design/styles/'.htmlspecialchars($api_design).'/head.php';
/////////////////////////////////////////
?>
