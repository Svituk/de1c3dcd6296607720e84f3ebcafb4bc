...
echo '<div class="apicms_subhead"><table width="100%" ><tr><td width="10%"><center><img src="/design/styles/'.display_html($api_design).'/lib/article.png">';
echo "</center></td><td width='90%'>";
$user_level = isset($user['level']) ? intval($user['level']) : 0;
$user_id = isset($user['id']) ? intval($user['id']) : 0;
if ($user_level==1 || ($user_id && $user_id==$post_lib['id_user'])) echo ' <span style="float:right"><a href="delete_article.php?id='.$post_lib['id'].'&raz='.$lib_id.'"><img src="/design/styles/'.display_html($api_design).'/lib/del_article.png"></a> </span>';
echo "<a href='article.php?id=$post_lib[id]'><b>".display_html($post_lib['name'])."</b></a>";
