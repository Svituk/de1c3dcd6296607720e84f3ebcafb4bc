...
if (isset($_GET['token']) && isset($_GET['uid'])){
    ...
    $row = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM `password_resets` WHERE `user_id` = '".$uid."' AND `token` = '".mysqli_real_escape_string($connect,$token)."' AND `expires` >= '".time()."' LIMIT 1"));
    if ($row && isset($_POST['newpass']) && isset($_POST['save']) && isset($_POST['csrf_token']) && hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'])){
        ...
        if (strlen($np) >= 8 && strlen($np) <= 64){
            ...
            mysqli_query($connect, "DELETE FROM `password_resets` WHERE `user_id` = '".$uid."'");
...
if (!isset($_GET['token'])){
    ...
    if ($login !== '' && $email !== '' && isset($_POST['csrf_token']) && hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'])){
        $qq = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM `users` WHERE `login` = '".apicms_filter($login)."' LIMIT 1"));
        if ($qq && isset($qq['login']) && isset($qq['email']) && $qq['login'] === $login && $qq['email'] === $email){
            ...
            $exp = time()+1800;
            mysqli_query($connect, "DELETE FROM `password_resets` WHERE `user_id` = '".intval($qq['id'])."'");
            ...
            $msg = $sent ? '<div class="apicms_subhead"><center>Ссылка для сброса пароля отправлена на e-mail '.htmlspecialchars($qq['login']).'</center></div>' : '<div class="erors"><center>Не удалось отправить письмо</center></div>';
        } else {
...
if (!isset($_GET['token'])){
    ...
    echo "<form action='?uid=".intval($_GET['uid'])."&token=".htmlspecialchars($_GET['token'])."' method='post'><div class='apicms_content'>";
