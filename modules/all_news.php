...
$author_login = (isset($ank['login']) && $ank['login'] !== null) ? htmlspecialchars($ank['login']) : '—';
$koms_display = ($koms == 0) ? '' : ' +'.intval($koms);
echo '<div class="apicms_subhead"><div class="descr"><font color=#ffffff><strong>'.apicms_smiles(apicms_bb_code(apicms_br(htmlspecialchars($newsone['name'])))).'</strong> <span style="float:right"><small>'.apicms_data($newsone['time']).'</small></span> <br/> '.apicms_smiles(apicms_bb_code(apicms_br(htmlspecialchars($newsone['txt'])))).' 
<hr><a href="/modules/news_comm.php?id='.$newsone['id'].'"><small>Комментарии '.$koms_display.'</small></a> <span style="float:right"> <small>'.$author_login.'</small> <img src="/design/styles/'.htmlspecialchars($api_design).'/menu/rsnews.png"/></span></font></div></div>';
}
/////////////////////////////////////////
if ($k_page > 1){
