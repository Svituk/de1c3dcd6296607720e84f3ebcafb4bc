...
while ($newsone = $query ? mysqli_fetch_assoc($query) : array()){
    ...
    echo ' <img src="/design/styles/'.htmlspecialchars($api_design).'/menu/news_m.png"/> <a href="/modules/all_news.php"><strong><font color=#ffffff>'.apicms_smiles(apicms_bb_code(apicms_br($name_safe))).'</font></strong></a> [ <a href="/modules/rss.php"><font color=#ffffff>RSS</font></a> ] <span style="float:right"><small> '.apicms_data(isset($newsone['time']) ? $newsone['time'] : time()).' </small></span> </br> '.apicms_smiles(apicms_bb_code(apicms_br($txt_safe))).'';
    echo '<hr> <img src="/design/styles/'.htmlspecialchars($api_design).'/menu/all_comms.png"/> <a href="/modules/news_comm.php?id='.$news_id.'"><small><font color=#ffffff>Комментарии '.$koms.'</font></small></a> <span style="float:right"> <small>'.$login_safe.'</small> <img src="/design/styles/'.htmlspecialchars($api_design).'/menu/rsnews.png"/></span> </div></div>';
}
///////////////////////////////////
?>
